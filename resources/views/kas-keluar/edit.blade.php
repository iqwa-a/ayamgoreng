<x-app-layout>
    <x-slot name="title">Edit KK</x-slot>
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 animate-[fadeIn_0.3s_ease-out]">
            <div>
                ...
                <div class="flex items-center gap-2 mb-2">
                    <a href="{{ route('kas-keluar.index') }}" class="w-6 h-6 rounded-full bg-stone-200 flex items-center justify-center hover:bg-amber-500 hover:text-white transition-all group">
                        <span class="material-symbols-rounded text-base">arrow_back</span>
                    </a>
                    <span class="h-1.5 w-1.5 rounded-full bg-amber-500"></span>
                    <p class="text-xs font-bold tracking-widest text-amber-600 uppercase">Mode Edit</p>
                </div>
                ...
                <p class="text-stone-500 text-sm mt-2 max-w-lg leading-relaxed font-medium">
                    Perbarui data transaksi jika terjadi kesalahan input atau penyesuaian biaya.
                </p>
            </div>
        </div>

        <form method="POST" action="{{ route('kas-keluar.update', $kasKeluar->id) }}" enctype="multipart/form-data" class="animate-[fadeIn_0.5s_ease-out]">
            @csrf
            @method('PUT')
            ...
            @if ($errors->any())
                <div class="mb-8 bg-rose-50 border border-rose-100 text-rose-600 px-6 py-4 rounded-[1.5rem] shadow-sm flex items-start gap-3">
                    ...
                    <div>
                        <h3 class="font-bold text-sm mb-1">Periksa Inputan Anda</h3>
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="bg-white rounded-[2.5rem] p-6 md:p-8 shadow-soft border border-stone-100 relative overflow-hidden group hover:border-amber-300/30 transition-all duration-500">
                        ...
                        <div class="relative z-10">
                            ...
                            <div class="relative">
                                <span class="absolute left-6 top-1/2 -translate-y-1/2 text-stone-400 text-2xl md:text-3xl font-bold group-focus-within:text-amber-600 transition-colors">Rp</span>
                                <input type="text" x-model="displayNominal" @input="updateNominal" placeholder="0" required
                                       class="w-full bg-stone-50 border-transparent focus:border-amber-500/50 rounded-[2rem] py-6 pl-16 md:pl-20 pr-6 text-3xl md:text-5xl font-black text-stone-800 focus:bg-white focus:ring-4 focus:ring-amber-500/10 transition-all outline-none placeholder-stone-200 tracking-tight shadow-inner">
                                    ...
                                    <div class="relative" @click.outside="kategoriOpen = false">
                                        ...
                                        <div x-show="kategoriOpen" x-transition.opacity.duration.200ms style="display: none;"
                                            ...
                                            <template x-for="option in filteredOptions" :key="option">
                                                <button type="button" @click="search = option; kategoriOpen = false"
                                            ...
                                            <div x-show="filteredOptions.length === 0 && search.length > 0"
                                                ...
                                                Kategori baru: <br>
                                                <span x-text="search" class="font-bold text-amber-600 not-italic"></span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="relative">
                                    ...
                                    <input type="text" name="penerima" value="{{ old('penerima', $kasKeluar->penerima) }}" required
                                           class="w-full bg-stone-50 border-transparent focus:border-amber-500/50 rounded-[1.5rem] text-stone-700 font-bold focus:bg-white focus:ring-4 focus:ring-amber-500/10 transition-all py-3.5 pl-12 pr-4 text-sm shadow-inner placeholder:font-normal">
                                </div>
                            </div>
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="relative group cursor-pointer">
                                    ...
                                    <div class="border-2 border-dashed border-stone-200 rounded-[1.5rem] p-4 flex items-center gap-4 transition-all group-hover:border-amber-400 group-hover:bg-amber-50/30"
                                         :class="imageUrl ? 'bg-stone-50 border-solid border-stone-300' : 'bg-stone-50/50'">

                                        {{-- Placeholder --}}
                                        <div class="flex items-center gap-4 w-full" x-show="!imageUrl">
                                            <div class="w-12 h-12 rounded-2xl bg-white border border-stone-200 flex items-center justify-center text-stone-300 group-hover:text-amber-500 group-hover:border-amber-200 transition-all shadow-sm">
                                                <span class="material-symbols-rounded text-2xl">add_a_photo</span>
                                            </div>
                                            <div>
                                                <p class="text-sm font-bold text-stone-600 group-hover:text-amber-600 transition-colors">Ganti Bukti</p>
                                        ...
                                        <div class="flex items-center gap-4 w-full" x-show="imageUrl" style="display: none;">
                                            ...
                                            <div class="flex-1 min-w-0">
                                                ...
                                                <p class="text-[10px] text-amber-500 font-bold">Klik untuk ganti</p>
                                            </div>
                                            <button type="button" @click.prevent="removeImage" class="z-30 p-2 rounded-full bg-white border border-stone-200 text-stone-400 hover:text-amber-500 hover:border-amber-200 transition-all shadow-sm">
                            ...
                            <div class="relative z-10 flex flex-col h-full justify-between min-h-[160px]">
                                ...
                                <div class="mt-6 pt-4 border-t border-white/20 flex justify-between items-center">
                                    <div class="flex flex-col max-w-[75%]">
                                        <span class="text-[10px] text-orange-100/70 font-bold uppercase tracking-wide">Sumber Dana</span>
